...
contract Identity is MessageSigned, ERC725, ERC735 {
    ...
    modifier managementOnly {
        if(msg.sender == address(this)) { 
            ...
            //request execution
            keyRequestExecute(keccak256(abi.encode(msg.sender)), address(this), 0, msg.data);
        }
    }
    ...
    modifier keyMessageSigned (
        ...
        require(_key == keccak256(abi.encode(recoverAddress(getSignHash(_messageHash),_signature),salt)));
        _;
    }
    ...
    constructor(   
        ...
        if (_keys.length == 0) {
            ...
            initKeys[0] = keccak256(abi.encode(msg.sender));
    ...
    {
        executionId = keyRequestExecute(keccak256(abi.encode(msg.sender)), _to, _value, _data);   
    }
    ...
    {   
        return keyApproveExecute(keccak256(abi.encode(msg.sender)), _executionId, _approval);
    }
    ...
    function executeMessageSigned(
        ...
        keyMessageSigned(
            ...
            keccak256(
                abi.encode(
                    address(this), 
                    bytes4(keccak256(abi.encode("execute(address,uint256,bytes)"))), 
                    _to,
                    _value,
                    _data,
                    _nonce
        ...
        managementOnly
    {
        ...
        require(_minimumApprovals <= keysByPurpose[keccak256(abi.encode(_purpose, salt))].length);
        ...
        returns (bytes32 claimHash)
    {
        ...
        if (msg.sender == address(this)) {
            ...
            require(keyHasPurpose(keccak256(abi.encode(msg.sender)), CLAIM_SIGNER_KEY));
    ...
    {
        Key storage myKey = keys[keccak256(abi.encode(_key, salt))];
    ...
    function keyHasPurpose(bytes32 _key, uint256 _purpose) 
        ...
        returns (bool exists) 
    {
        return isKeyPurpose[keccak256(abi.encode(keccak256(abi.encode(_key, salt)), _purpose))];
    }
    ...
    {
        return keys[keccak256(abi.encode(_key, salt))].purposes;
    }
    ...
    function getKeysByPurpose(uint256 _purpose)
        ...
        returns(bytes32[])
    {
        return keysByPurpose[keccak256(abi.encode(_purpose, salt))];
    }
        ...
        internal 
    {
        ...
        require(keysByPurpose[keccak256(abi.encode(MANAGEMENT_KEY, _salt))].length == 0, "Already Initialized (2)");
        ...
        returns(bool success)
    {
        ...
        bytes32 keySaltedHash = keccak256(abi.encode(_key, _salt)); //key storage pointer
        bytes32 saltedKeyPurposeHash = keccak256(abi.encode(keySaltedHash, _purpose)); // accounts by purpose hash element index pointer
        ...
        if (keys[keySaltedHash].key == 0) { //is a new key
            ...
            bytes32 keyPurposeSaltedHash = keccak256(abi.encode(_key, _purpose, _salt)); //index of purpose in key pointer
        ...
        private 
    {
        ...
        bytes32 keyPurposeSaltedHash = keccak256(abi.encode(_key, _purpose, _salt)); //account purpose array element index
        ...
        if (replacerPurposeIndex > 1) { //is not the last key
            ...
            if(removedPurposeIndex != replacerPurposeIndex) { //removed element is not last element
                ...
                indexes[keccak256(abi.encode(_key, replacerPurpose, _salt))] = removedPurposeIndex; //update index
            }
    ...
    function dropKeyFromPurposesList(
        ...
        private
    {
        bytes32 purposeSaltedHash = keccak256(abi.encode(_purpose, _salt)); // salted accounts by purpose array index pointer   
        ...
        if (removedKeyIndex != replacerKeyIndex) {  // deleted not the last element, replace deleted by last element
            ...
            indexes[keccak256(abi.encode(keccak256(abi.encode(replacerKey, _salt)), _purpose))] = removedKeyIndex; // update saltedKeyPurposeHash index of replacer
        }
    ...
    function overwriteKey(
        ...
        returns (bool success)
    {   
        bytes32 newKeySaltedHash = keccak256(abi.encode(_newKey, _salt)); // key storage pointer     
        if (_oldKey == _newKey) { //not replacing key, just keyType
            keys[newKeySaltedHash].keyType == _newType; 
            return true;
        }
        bytes32 oldKeySaltedHash = keccak256(abi.encode(_oldKey, _salt)); // key storage pointer     
    ...
    {
        bytes32 purposeSaltedHash = keccak256(abi.encode(_purpose, _salt)); // salted accounts by purpose array index pointer   
        bytes32 saltedOldKeyPurposeHash = keccak256(abi.encode(oldKeySaltedHash, _purpose)); // accounts by purpose hash element index pointer
        bytes32 saltedNewKeyPurposeHash = keccak256(abi.encode(newKeySaltedHash, _purpose)); // accounts by purpose hash element index pointer
        bytes32 oldKeyPurposeSaltedHash = keccak256(abi.encode(_oldKey, _purpose, _salt)); //account purpose array element index
        bytes32 newKeyPurposeSaltedHash = keccak256(abi.encode(_newKey, _purpose, _salt)); //account purpose array element index
