...
contract IdentityCertifier {
    ...
    event IdentityCreated(address indexed ad, uint256 gas);
    event IdentityRecharged(address indexed ad, uint256 gas);

    modifier hasEnoughBalance(uint256 gas) {
        require (msg.value >= gas || this.balance >= gas);
        _;
    }
    ...
    public payable hasEnoughBalance(_gas) returns (address res)
    {
